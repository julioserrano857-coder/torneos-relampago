    ...
    try {
      ...
      if (error) {
        if (error.message.includes('already registered')) {
          toast.error('Este email ya está registrado. Intentá ingresar.')
        } else {
        ...
        <Card className="bg-white/10 backdrop-blur-sm border-emerald-700/50 w-full max-w-md">
          <CardContent className="p-6 space-y-6">
            <div className="text-center">
              ...
              <p className="text-emerald-300 text-sm mt-1">Empezá a organizar torneos gratis</p>
            </div>

            <form onSubmit={handleRegister} className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="name" className="text-emerald-300">Tu nombre</Label>
              ...
              >
                {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : (
                  <>Crear Cuenta <ArrowRight className="h-5 w-5 ml-2" /></>
                )}
              </Button>
            </form>
            ...
            <div className="text-center">
              <p className="text-emerald-400 text-sm">
                ¿Ya tenés cuenta?{' '}
                <Link href="/login" className="text-emerald-300 underline font-medium">
                  Ingresá
                </Link>
              </p>
            </div>
